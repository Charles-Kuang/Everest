...
parser.add_argument('--save', '-s', default='./output', type=str, help='directory for saving')
...
tools.add_arguments_for_module(parser, datasets, argument_for_class='inference_dataset', default='ImagesFromFolder', 
                                ...
                                parameter_defaults={'replicates': 1})
...
with tools.TimerBlock("Parsing Arguments") as block:
    ...
    if args.inference:
        args.total_epochs = 1
        args.inference_dir = "{}".format(args.save)
...
def inference(args, epoch, data_loader, model, offset=0):
    ...
    if args.save_flow:
        flow_folder = "{}".format(args.save,args.name.replace('/', '.'),epoch)
        if not os.path.exists(flow_folder):
            os.makedirs(flow_folder)
    
    # visualization folder
    if args.inference_visualize:
        flow_vis_folder = "{}".format(args.save, args.name.replace('/', '.'), epoch)
